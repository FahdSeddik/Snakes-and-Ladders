...
public:
	...
	Cell* GetCellWithPos(CellPosition pos); // to get cell
	// ========= Other Getters =========
	...
	void setCurrentPlayer(int = 0);
	int GetCardNineCount()const;
	int GetCardTenCount()const;
	int GetCardElevenCount()const;
	void LightningOtherPlayers(Player* callingp);
	void DoSpecial(Player*);
	Player* GetPlayerwithNumber(int)const;
	// ========= User Interface Functions =========
	void ClearGrid();
	void SkipTurnofPlayer(int pnum);
	...
	void SaveAll(ofstream& OutFile, string filename);
	void LoadGrid(ifstream& InFile, string filename);
