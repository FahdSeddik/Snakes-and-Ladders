...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum < 0 || playerNum>MaxPlayerCount || !(cellPos.IsValidCell()))
	{
		return;
	}
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.HCell() != toCell.HCell()) {
		//PrintMessage("Startcell(HCell) not equal Endcell(HCell).");
		return;
	}
	else if (fromCell.VCell() <= toCell.VCell()){
		//PrintMessage("Endcell(VCell) bigger than or equal Startcell(VCell).");
		return;
	}
	else if (fromCell.VCell() == NumVerticalCells - 1) {
		//PrintMessage("Startcell(Vcell) cannot be bottom row.");
		return;
	}
	...
	/*else if (fromCell.HCell() == toCell.HCell() || toCell.VCell() < fromCell.VCell())*/
	...
	int horLinesNum = fromCell.VCell() - toCell.VCell();
	for (int i = 1; i <= horLinesNum; i++) {
		int yposLine = GetCellStartY(toCell) + UI.CellHeight * i;
		pWind->DrawLine(x12, yposLine, x34, yposLine);
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (toCell.HCell() != fromCell.HCell()) {
		//PrintMessage("Startcell(HCell) not equal Endcell(Hcell).");
		return;
	}
	else if (toCell.VCell() <= fromCell.VCell()) {
		//PrintMessage("Startcell(VCell) bigger than or equal Endcell(VCell).");
		return;
	}
	else if (fromCell.VCell() == NumVerticalCells - 1) {
		//PrintMessage("Startcell(Vcell) cannot be bottom row.");
